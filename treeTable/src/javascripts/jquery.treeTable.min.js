...
initDragDrop($(this));return _return;};$.fn.treeTable.defaults={childPrefix:"child-of-",expandable:true,indent:19,initialState:"collapsed",treeColumn:0,draggable:false,dragTarget:"tbody tr td .drag_handle",dropTarget:"tbody tr td",sortable:false};$.fn.collapse=function(){$(this).addClass("collapsed");childrenOf($(this)).each(function(){initialize($(this));if(!$(this).hasClass("collapsed")){$(this).collapse();}
...
return this;};$.fn.parentOf=function(){var classNames=$(this)[0].className.split(' ');for(key in classNames){if(classNames[key].match("child-of-")){return $("#"+classNames[key].substring(9));}}};function ancestorsOf(node){var ancestors=[];while(node=parentOf(node)){ancestors[ancestors.length]=node[0];}
...
ghost_row.hide();$($(ui.draggable).parents("tr")).appendBranchTo(this);},hoverClass:"accept",over:function(e,ui){if(options.sortable)
...
{$("tr td span.ghost_text").each(function(){$(this).parents("tr").droppable({accept:options.dragTarget,drop:function(e,ui){ghost_row.hide();var node=$(ui.draggable.parents("tr"));node.moveBranchAfter($(this).prev("tr"));}});});}};function add_expandable_widget(node)
